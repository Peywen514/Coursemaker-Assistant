...
// Dynamic Date Logic (Intelligent Year Selection)
const now = new Date();
const currentYear = now.getFullYear();
const currentMonth = now.getMonth(); // 0-indexed (0=Jan, 9=Oct)

// Logic: If currently Oct (9), Nov (10), or Dec (11), focus on NEXT YEAR.
// Otherwise, focus on CURRENT YEAR.
const isLateYear = currentMonth >= 9; 
const marketingFocusYear = isLateYear ? currentYear + 1 : currentYear;

// Context string for AI instructions
const marketContextDescription = isLateYear
  ? `It is currently late ${currentYear} (Q4). The market focus is now on **${marketingFocusYear}**. People are planning for the upcoming year.`
  : `It is currently ${currentYear}. The market focus is on immediate **${marketingFocusYear}** trends.`;
 ...
 * Updated with intelligent year logic.
 */
export const analyzeCourseAndGetPainPoints = async (course: CourseInfo): Promise<PainPoint[]> => {
  ...
  const prompt = `
    ...
    Your goal is to maximize SEO traffic and course enrollment.

    **TIME CONTEXT:**
    ${marketContextDescription}
    
    **CRITICAL YEAR REQUIREMENT:**
    - **ALWAYS** use **${marketingFocusYear}** in your Titles and Keywords.
    - **DO NOT** use ranges like "${currentYear}~${marketingFocusYear}".
    - If it's late year, frame it as "Preparing for ${marketingFocusYear}" (佈局${marketingFocusYear}).
    - If it's early year, frame it as "${marketingFocusYear} Latest Trends" (${marketingFocusYear}最新趨勢).
    ...
    1. **Job Seekers/Unemployed (待業者/轉職者):** Focus on hiring trends for ${marketingFocusYear}, modern resume standards (ATS friendly).
    2. **Employed Professionals (在職者):** Focus on Upskilling, Salary negotiation, and burnout.
    3. **Students/Fresh Grads (學生/新鮮人):** Focus on "Graduation Anxiety" and lack of practical experience.
    4. **Re-entering Workforce (二度就業者):** Focus on confidence building and catching up with digital tools.

    For each angle, identify **Trending SEO Keywords** that are popular in Taiwan searches.
      ...
      responseSchema: {
        ...
        items: {
          ...
          properties: {
            ...
            title: { type: Type.STRING, description: `A catchy title strictly using year ${marketingFocusYear}` },
            description: { type: Type.STRING, description: `Why this group needs this course for ${marketingFocusYear}` },
...
export const generateLazyPackContent = async (course: CourseInfo, painPoint: PainPoint): Promise<SlideContent[]> => {
  ...
  const prompt = `
    Create content for a 5-slide Instagram/Facebook carousel (Lazy Pack/懶人包) for the Taiwan market.
    ...
    **TIME CONTEXT:**
    ...
    Style Guide:
    - **DATE RULE**: STRICTLY USE **"${marketingFocusYear}"**. Do NOT use ranges (e.g., "${currentYear}-${marketingFocusYear}").
    - Use modern Taiwan social media slang (e.g., 必收, 乾貨, 焦慮, 救星, 懂的都懂).
    - Tone: Empathetic, fast-paced, "Helpful Peer" persona.
    - Structure:
      Slide 1: **Viral Cover**. High impact title. Must include "**${marketingFocusYear}**". (e.g., "${marketingFocusYear}轉職必看!", "${marketingFocusYear}必學").
      Slide 2: **The Current Pain**. Empathize with a specific struggle.
...
export const generateVideoScript = async (course: CourseInfo, painPoint: PainPoint): Promise<VideoScriptScene[]> => {
    ...
    const prompt = `
      ...
      **TIME CONTEXT:**
      ...
      Script Requirements:
      1. **0-3s Hook**: Must stop the scroll. Use "**${marketingFocusYear}**" to create urgency (e.g., "${marketingFocusYear}薪資趨勢...").
